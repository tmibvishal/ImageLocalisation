...
def main():
    ...
    img1 = cv.imread("testData/testing/image0 (1).jpg", cv.IMREAD_GRAYSCALE)
    img2 = cv.imread("testData/testing/image0.jpg", cv.IMREAD_GRAYSCALE)
    ...
    def match_and_draw(win):
        # with Timer('matching'):
        raw_matches1 = matcher.knnMatch(desc1, trainDescriptors = desc2, k = 2)
        print("length of raw features is "+str(len(raw_matches1)))
        raw_matches2= matcher.knnMatch(desc2, trainDescriptors = desc1, k = 2)
        p1, p2, kp_pairs1 = filter_matches(kp1, kp2, raw_matches1)
        p3, p4, kp_pairs2 = filter_matches(kp2, kp1, raw_matches2)
        a1= len(kp_pairs1)
        a2= len(kp_pairs2)
        if len(p1) >= 4:
            ...
            kp_pairs1 = [kpp for kpp, flag in zip(kp_pairs1, status) if flag]
            b1= len(kp_pairs1)
        else:
            H, status = None, None
            print('%d matches found, not enough for homography estimation' % len(p1))
        if len(p3) >= 4:
            H, status = cv.findHomography(p3, p4, cv.RANSAC, 5.0)
            # print('%d / %d  inliers/matched' % (np.sum(status), len(status)))
            # do not draw outliers (there will be a lot of them)
            kp_pairs2 = [kpp for kpp, flag in zip(kp_pairs2, status) if flag]
            b2= len(kp_pairs2)
        else:
            H, status = None, None
            print('%d matches found, not enough for homography estimation' % len(p1))

        # explore_match(win, img1, img2, kp_pairs1, None, H)
        print("percentage match is "+str((b1+b2)/(a1+a2)))
        # match= c/(len(kp1)+len(kp2)
        # print(match)
