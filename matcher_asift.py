...
def matcher(img1 , img2, feature_name= "surf-flann",symmetry_match: bool = True):
    ...
    :param img1:
    :param img2:
    :param feature_name: of the type a-flann,  where a is feature to use which can be sift, surf, orb or brisk. Append '-flann'
               to feature name to use Flann-based matcher instead bruteforce.
    :param symmetry_match:
    :return:
    """
...
def matcher_2(key_des_1,key_des_2, feature_name= "surf-flann", symmetry_match: bool = True):

    if key_des_1 is None or key_des_2 is None:
        raise Exception("key_des_1 or key_des_1 can't be none")
        return -1

    len_keypoints1, descriptors1, keypoints1 = key_des_1
    len_keypoints2, descriptors2, keypoints2 = key_des_2
    ...
    if (symmetry_match):
        raw_matches1 = matcher.knnMatch(descriptors2, trainDescriptors=descriptors1, k=2)
        p3, p4, kp_pairs1 = filter_matches(keypoints2, keypoints1, raw_matches1)
        a2 = len(kp_pairs1)
